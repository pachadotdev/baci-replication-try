    ...
    mutate(
      fobization = case_when(
        ...
        qty_unit_exp == "electrical energy in thousands of kilowatt-hours" |
          qty_unit_imp == "electrical energy in thousands of kilowatt-hours" ~ "no fobization",  
        TRUE ~ fobization
      )
    )
}
  ...
  d %>% 
    ...
    mutate(
      difference_result = case_when(
        fobization_mismatch >= direct_mismatch & fobization == "fobization" ~ "use exports",
        fobization_mismatch < direct_mismatch & fobization == "fobization" ~ "use imports",
        TRUE ~ NA_character_
      )
    )
}
  ...
  d %>% 
    mutate(
      reported_value = case_when(
        ...
        reported_by == "importer only" & fobization == "no fobization" ~ "imports, no change",
        reported_by == "importer only" & fobization == "fobization" ~ "imports, divided by cif/fob ratio",
        reported_by == "both parties" & fobization == "no fobization" ~ "exports, no change",
        reported_by == "both parties" & fobization == "fobization" & difference_result == "use exports" ~ "exports, no change",
        reported_by == "both parties" & fobization == "fobization" & difference_result == "use imports" ~ "imports, divided by cif/fob ratio"
      ),
      ...
      trade_value_usd_exp = case_when(
        ...
        reported_value == "imports, divided by cif/fob ratio" ~ trade_value_usd_imp_fob
      )
    )
}
