...
fit <- lm(
  log(cif_fob_ratio) ~ log(dist) + log(uv_exp) + contig +
...
n <- nrow(fit$model)
outliers <- number_outliers(fit)

model3 <- list(fit = fit, outliers = outliers)
rm(fit, n, p, outliers); gc()

try(dir.create("models"))

fout <- "models/model3.rds"
saveRDS(model3, fout, compress = "xz")
